...
To (re)link every non-deprecated skill into the local harness skill directories (`~/.claude/skills`, `~/.agents/skills`), manually run `scripts/link-skills.sh`. This is an attended write outside the repo: it creates or refreshes links into this repo and prunes dangling links whose targets are inside this repo. It preserves real directories and unrelated entries, and exits with an error when one blocks a repo skill. A `git pull` keeps existing linked skills current; re-run the script manually after adding, removing, or renaming a skill.
