...
[Source](https://github.com/jdpretzel/project-home/tree/main/skills/engineering/wayfinder)

## What it does

`wayfinder` takes an effort too big for one agent session — wrapped in fog, where the way from here to the goal isn't visible yet — and charts it as a **shared map** of **decision tickets** on GitHub, then resolves them one at a time until the way is clear. It **plans, it doesn't do**: every ticket resolves a decision — a question to settle, not a slice of a build to execute — and the map is done when nothing is left to decide before someone goes and builds the thing — so it produces decisions, not deliverables.
...
A GitHub repo you can reach. The map and its tickets *are* GitHub issues, so wayfinder needs a GitHub remote, an authenticated `gh`, and Issues enabled — and if one of those is missing it stops and tells you which, rather than quietly writing your map somewhere else. Nothing else needs configuring: the skill carries its own recipes for sub-issues, dependencies, claiming, and the frontier query. Where a repo doesn't have sub-issues or issue dependencies, it falls back to a task list in the map body and `Blocked by: #<n>` lines in the tickets.
