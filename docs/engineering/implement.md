...
`implement` builds the work described in a spec or a set of tickets — driving it through test-driven development, typechecking, and the full test suite, then committing, reviewing that commit, and opening a pull request. It runs unattended from end to end; the only thing it stops for is your decision to merge.
...
```txt
grill-with-docs → to-spec → to-tickets → implement (tdd → commit → code-review → PR)
```
