...
[Source](https://github.com/jdpretzel/project-home/tree/main/skills/engineering/prototype)
...
Picking the wrong branch wastes the whole prototype, so the question comes first. Both branches keep state in memory, are trivial to run, and surface the full state on every step. And where the repo provides a lifecycle entry — a script like `scripts/agent-lifecycle.sh` that starts work in an isolated worktree — both are built in a worktree it started: throwaway code is still code, and it never mutates the primary checkout.
