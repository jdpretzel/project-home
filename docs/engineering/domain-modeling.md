...
[Source](https://github.com/jdpretzel/project-home/tree/main/skills/engineering/domain-modeling)
...
The skill writes into two places, both created lazily — only once there is something to record. Resolved terms go into `CONTEXT.md` at the root (or, in a multi-context repo flagged by a `CONTEXT-MAP.md`, into the per-context `CONTEXT.md`). Decisions go into `docs/adr/`, or into whatever ADR path [setup-project-home](https://aihero.dev/skills-setup-project-home) has recorded in your root instructions — a recorded path overrides the default for both reading and writing, so the repo never ends up with two ADR sequences. Nothing needs to exist up front; the first resolved term creates the glossary, the first real trade-off creates the ADR.
...
- **An ADR** captures a decision, and the bar is high: offered only when the choice is **hard to reverse**, **surprising without context**, and **the result of a real trade-off**. Miss any one of the three and there is no ADR. This is what keeps the ADR log a record of consequential forks rather than a diary.
