...
Move this repo's GitHub issues through a small state machine of triage roles.
...
Query the repo's issues (`gh issue list --state open --json number,title,body,labels,comments`, with `--label` filters as needed) and present three buckets, oldest first:
